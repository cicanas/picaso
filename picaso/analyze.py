...
class GridFitter(): 
    ...
    def fit_grid(self,grid_name, data_name,dof='ndata', offset=True, datalabels = None, minwl = None, offsetwl = None, spectralref = None):
        ...
        self.lnls =  getattr(self, 'lnls',{grid_name: {data_name:np.zeros(shape=(nmodels))}})
        ...
        self.lnls[grid_name] = self.lnls.get(grid_name, {data_name:np.zeros(shape=(nmodels))})
        ...
        self.lnls[grid_name][data_name] = self.lnls[grid_name].get(data_name, np.zeros(shape=(nmodels)))
        ...
        for index in range(nmodels):
            ...
            self.lnls[grid_name][data_name][index]= gaussian_log_likelihood(data = y_data, data_err = e_data, model = flux_in_bin+shift)
